...
export interface ProfitabilitySnapshotContext {
...
export class ProfitabilitySnapshotAdapter {
  public build(
    context: ProfitabilitySnapshotContext
  ): PrismModuleSnapshot {
    const metrics =
      context.registry.getMetrics("Profitability");

    const thresholds =
      context.registry.getThresholds("Profitability");

    const statuses =
      context.registry.getStatuses("Profitability");

    const narratives =
      context.registry.getNarratives("Profitability");

    const reportingRows =
      this.getReportingRows(context.isRows);
    ...
    const previousEoyEntry =
      latestEntry
        ? this.getPreviousYearEndRow(
            reportingRows,
            latestEntry.date
          )
        : undefined;
    ...
    const previousEoyRow =
      previousEoyEntry?.row ?? {};
    ...
    return {
      ...
      analytics: {
        incomeStatement: {
          history: reportingRows.map((entry) => entry.row),
        },
        nim: {
