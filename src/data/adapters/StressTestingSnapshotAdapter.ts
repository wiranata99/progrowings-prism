...
export interface StressTestingSnapshotContext {
  registry: SchemaRegistry;
  ewiRows: DatabaseRow[];
}

export class StressTestingSnapshotAdapter {
  public build(
    context: StressTestingSnapshotContext
  ): PrismModuleSnapshot {
    const latestRow = this.getLatestRow(context.ewiRows);

    const metrics = context.registry.getMetrics("Stress Testing");
    const thresholds = context.registry.getThresholds("Stress Testing");
    const statuses = context.registry.getStatuses("Stress Testing");
    const narratives = context.registry.getNarratives("Stress Testing");
