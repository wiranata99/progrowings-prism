import { useEffect, useState } from "react";

import {
  getCreditExecutiveNarrative,
  type CreditExecutiveNarrative,
} from "../../services/creditMomentumApi";

export default function ExecutiveNarrative() {
  const [narrative, setNarrative] = useState<CreditExecutiveNarrative | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isUnavailable, setIsUnavailable] = useState(false);

  useEffect(() => {
    const controller = new AbortController();
    getCreditExecutiveNarrative(controller.signal)
      .then(setNarrative)
      .catch((error: unknown) => {
        if (error instanceof DOMException && error.name === "AbortError") return;
        setIsUnavailable(true);
      })
      .finally(() => setIsLoading(false));
    return () => controller.abort();
  }, []);

  return (
    <section className="rounded-3xl border border-cyan-500/20 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-950 p-8">
      {isLoading && (
        <div className="h-64 animate-pulse rounded-2xl bg-slate-800" />
      )}

      {!isLoading && (isUnavailable || !narrative) && (
        <div className="rounded-2xl border border-slate-700 bg-slate-950/60 p-8 text-center text-slate-400">
          Executive narrative is currently unavailable.
        </div>
      )}

      {!isLoading && narrative && (
        <>
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.30em] text-cyan-400">
              Executive Brief
            </p>
            <h2 className="mt-3 text-4xl font-bold tracking-tight text-white">
              {narrative.title}
            </h2>
          </div>
          <div className="mt-8">
            <p className="max-w-5xl text-lg leading-9 text-slate-300">
              {narrative.summary}
            </p>
          </div>
          <div className="mt-10 border-t border-slate-800 pt-8">
            <h3 className="text-sm font-semibold uppercase tracking-[0.25em] text-cyan-400">
              Recommended Actions
            </h3>
            <div className="mt-5 space-y-4">
              {narrative.actions.map((action, index) => (
                <div key={action} className="flex items-start gap-4">
                  <div className="mt-1 flex h-6 w-6 items-center justify-center rounded-full bg-cyan-500/15 text-sm font-bold text-cyan-400">
                    {index + 1}
                  </div>
                  <p className="leading-7 text-slate-300">{action}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="mt-10 flex items-center justify-between border-t border-slate-800 pt-6">
            <p className="text-sm text-slate-500">
              Estimated reading time <span className="font-semibold text-slate-300">45 sec</span>
            </p>
            <div className="rounded-full bg-emerald-500/15 px-4 py-2">
              <span className="text-sm font-semibold text-emerald-400">Board Ready</span>
            </div>
          </div>
        </>
      )}
    </section>
  );
}
